...
const binanceBegin = () => binance.options({
...
describe('Account snapshot', function () {
    // eslint-disable-next-line no-undef
    before(binanceBegin);
    it('Gets the account SPOT snapshot', async () => {
        try {
            const snapshot = await binance.accountSnapshot();
            debug(snapshot);
            assert(snapshot !== null, WARN_SHOULD_BE_NOT_NULL);
            assert(snapshot);
            assert(Object.prototype.hasOwnProperty.call(snapshot, 'snapshotVos'), WARN_SHOULD_HAVE_KEY + 'snapshotVos');
            assert(Array.isArray(snapshot.snapshotVos));
            assert(snapshot.snapshotVos.length > 0);
            snapshot.snapshotVos.forEach(function (snap) {
                assert(Object.prototype.hasOwnPorperty.call(snap, 'type'));
                assert(Object.prototype.hasOwnPorperty.call(snap, 'updateTime'));
                assert(Object.prototype.hasOwnPorperty.call(snap, 'data'));
            });
        } catch (err) {
            debug(err.message);
        }
...
describe('Sell Stop loess', function () {
    it('Attempt to create a stop loss order', function (done) {
        ...
        let price = 0.069;
        let stopPrice = 0.068;
        assert(typeof (binance.sell('ETHBTC', quantity, price, {
            stopPrice: stopPrice,
            type: type
        })) === 'undefined', WARN_SHOULD_BE_UNDEFINED);
        done();
    }).timeout(TIMEOUT);
});
...
describe('Reverse', function () {
    it('Reverse the keys in an object', function (done) {
        assert(binance.reverse({ '3': 3, '2': 2, '1': 1 }).toString() === {
            '1': 1,
            '2': 2,
            '3': 3
        }.toString(), 'should be {\'1\': 1, \'2\': 2, \'3\': 3 }');
        done();
    }).timeout(TIMEOUT);
});
...
describe('ohlc', function () {
    let chart = [{
        ...
        'close': 4.44444,
        'volume': 3.333333
    }, { 'open': 1.11111, 'high': 6.6666, 'low': 8.88888, 'close': 4.44444, 'volume': 3.333333 }];
